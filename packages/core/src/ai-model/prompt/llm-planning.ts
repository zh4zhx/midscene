...
**Special case - Loading, waiting, or processing states:**
- If the current screenshot shows a temporary in-progress state after a previous action (for example loading, waiting, processing, uploading, exporting, importing, generating, syncing, a spinner, progress bar, or a message such as "please wait"), the task is not failed yet.
- In that situation, do NOT output <complete success="false"> just because the next required UI is not visible yet.
- If a Sleep action is available, output a Sleep action to wait briefly, then observe the next screenshot and continue planning. Use <complete success="false"> only when the screenshot or history proves the task cannot continue, or the same unrecoverable state persists after repeated waits.
